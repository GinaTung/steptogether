<template>
  <Panel class="p-5 mb-5" v-for="postsData in postsList" :key="postsData.id">
    <template #header>
    ...
    <template #footer>
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-2">
          <Button
            :icon="postStates[postsData.id]?.isHeartVisible ? 'pi pi-heart-fill' : 'pi pi-heart'"
            @click="toggleHeart(postsData.id)"
          ></Button>

          <Button
            :icon="postStates[postsData.id]?.isCommentVisible ? 'pi pi-comments' : 'pi pi-comment'"
            severity="secondary"
            rounded
            @click="toggleComment(postsData.id)"
          ></Button>

          <Button
            icon="pi pi-send"
            v-if="postsData.status !== '已發布'"
            :class="{ 'filled-icon': postStates[postsData.id]?.isSendVisible }"
            severity="secondary"
            @click="toggleSend(postsData.id)"
          ></Button>
        </div>
      ...
      <div v-if="postStates[postsData.id]?.isCommentVisible">
        <div class="flex items-center">
          ...
          <Textarea
            ...
            rows="1"
            cols="30"
            class="w-full bg-[#d0d6dd26] border-1 rounded-lg p-3 my-3"
            placeholder="What’s on your mind ?"
          />
          <Button class="flex items-center cursor-pointer ms-3 hidden" 
          unstyled>
            <i class="pi pi-send"
            ></i>
          </Button>
    ...
    <div class="my-3">
      <div class="card">
        <Carousel 
          v-if="Array.isArray(postsData.image_url) && postsData.image_url.length > 0"
          :value="postsData.image_url"
          :numVisible="2" 
          :numScroll="1" 
            ...
            <div class="border border-surface-200 dark:border-surface-700 rounded m-2 p-4">
              <div class="mb-4">
                <div class="relative mx-auto">
                  <img :src="slotProps.data" alt="post image" class="w-full rounded" style="height: 300px;" />
                </div>
              </div>
            </div>
          </template>
        ...
        <h3 class="text-xl font-bold mb-2">{{ postsData.title }}</h3>
        <p class="text-justify">
          ...
          <Button
            ...
            label="more"
            @click="toggle"
            class="text-gray-700 ms-2 font-bold"
            style="--tw-text-opacity: 1; color: rgba(239,108,0,var(--tw-text-opacity));"
            @mouseover="e => e.target.style.color='#EF6C00'"
            @mouseleave="e => e.target.style.color='gray'"
          />
